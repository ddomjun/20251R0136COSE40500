...
def get_args_parser():
    parser = argparse.ArgumentParser('CLIP-DETR object detector', add_help=False)
...
def main(args):
    ...
    if utils.is_main_process():  # 분산 학습에서 중복 로깅 방지
        wandb.init(
            ...
            name=f"Clip-detr-{datetime.now().strftime('%Y%m%d-%H%M%S')}"  # 실행 이름
        )
    ...
    param_dicts = [
        ...
        {"params": [p for n, p in model_without_ddp.named_parameters()
               if "class_projection" in n or "bbox_projection" in n and p.requires_grad]},
        # Catch any remaining parameters that might not be covered
        {"params": [p for n, p in model_without_ddp.named_parameters()
                if not any(x in n for x in ["clip_projector.projection", "qformer", "query_embed", "class_projection", "bbox_projection"]) 
                and p.requires_grad]}
    ]
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if utils.is_main_process():
            eval_results = test_stats.get('coco_eval_bbox', [0, 0, 0])
            wandb.log({
                "val/epoch": epoch,
                "val/loss": test_stats['loss'],
                "val/mAP": eval_results[0] if len(eval_results) > 0 else 0,
                "val/mAP50": eval_results[1] if len(eval_results) > 1 else 0,
                "val/mAP75": eval_results[2] if len(eval_results) > 2 else 0
            })
    ########################
    if utils.is_main_process():
        wandb.finish()
